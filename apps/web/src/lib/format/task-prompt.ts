/** Format raw agent.prompt human payloads into readable text. Ported from apps/tui/src/task-prompt.ts. */
...
function formatQuestionRounds(value: unknown): string[] {
  ...
  for (let i = 0; i < value.length; i += 1) {
    ...
    for (let q = 0; q < questions.length; q += 1) {
      ...
      const question =
        typeof item === "string" ? item : isRecord(item) ? asString(item.question) : undefined;
...
function formatRequirementState(state: JsonRecord): string[] {
  ...
  if (score !== undefined) {
    const scoreLine = threshold !== undefined ? `完整度 ${score} / 目标 ${threshold}` : `完整度 ${score}`;
  ...
  pushSection(lines, "非功能需求", bulletLines(asStringList(state.nonFunctionalRequirements)));
...
function formatDevState(state: JsonRecord): string[] {
  ...
  const maxAttempts = typeof state.maxSliceAttempts === "number" ? state.maxSliceAttempts : undefined;
  if (attempts > 0 || maxAttempts !== undefined) {
    pushSection(lines, "重试", [
      maxAttempts !== undefined
        ? `当前切片第 ${attempts} / ${maxAttempts} 次尝试`
        : `当前切片第 ${attempts} 次尝试`,
    ]);
  }
