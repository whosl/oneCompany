 ...
 *
 * Ported verbatim from apps/tui/src/sse.ts. Uses fetch + ReadableStream reader
 * rather than EventSource because the API emits `data:`-only frames (no `id:`/
 * `event:`), needs `afterSeq` query-param replay, and sends ephemeral frames
 * with seq=0 that must pass through without advancing the cursor.
 */
