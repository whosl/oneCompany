 ...
 *
 * This is a faithful port of apps/tui/src/types.ts with two corrections: the
 * `ConsoleSnapshot.risks` field is included (the server returns it but the TUI
 * type omitted it), and `Readiness` is widened to match the real schema.
 */
...
export type ConsoleSnapshot = {
  ...
  /** Merged requirement + dev session risks (server returns it; TUI type omitted it). */
  risks?: string[];
  openGates: GateInfo[];
...
export type TaiziDispatchResult = {
  intent: string;
  action: string;
  reply: string;
