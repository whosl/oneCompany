...
/**
 * Full-surface HTTP client for the OneCompany API.
 *
 * Ported from apps/tui/src/api.ts. Talks only same-origin (/api/*); the
 * Next.js rewrite proxies requests to the API on :3001 so we never hit CORS.
 * No auth; workflow steps can block for minutes server-side, so keep a
 * generous timeout.
 */
export class ApiClient {
  ...
  exportSubmission(projectId: string): Promise<ExportResult> {
    return this.post(`/projects/${projectId}/delivery/export`, {}) as Promise<ExportResult>;
  }
   ...
   */
  interrupt(projectId: string, message: string, abort = false): Promise<{ delivered: boolean }> {
   ...
   * 立即返回路由结果；长动作进度走事件流。
   */
  taiziMessage(projectId: string, message: string): Promise<TaiziDispatchResult> {
    return this.post(`/projects/${projectId}/taizi/message`, { message }) as Promise<TaiziDispatchResult>;
  }
