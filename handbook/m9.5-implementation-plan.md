...
| Requirement API | 默认 LangChain runner（`ChatOpenAI.withStructuredOutput` + §13 `pickModel`），非 `runScriptedRequirementAgent` |
...
- [x] `development/service` 默认 `OpencodeHarness` + `createAuthorize` + 真实 `runAuthoritativeCheck`
- [x] `requirement/service` 与 `testing/service` 默认真实 agent runner
- [x] `OpencodeHarness.runSlice` 在产品中可用（M6 SDK 桥接：server / event / permission / log bridge）
- [x] 无生产路径 `allow:true` / `passed:true` / fixture `profile`
- [x] 缺 key / 不可用引擎按 §12 降级，不 fake success
- [x] opencode/model I/O redaction 有测试覆盖（`log-bridge` + workflow `harness-context`）
- [x] `OC_OPENCODE_INTEGRATION=1` golden-path 集成测（`integration/golden-path.test.ts`）
- [x] `pnpm -w test` + `pnpm -w typecheck` + `pnpm -w build` 绿
