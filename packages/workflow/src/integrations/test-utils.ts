...
import { seedDefaultIntegrations } from "@oc/integrations";
import { createDb, projects, type Db } from "@oc/shared";

export function seedIntegrationTestProject(
  db: Db,
...
export function setupIntegrationTestDb(): { db: Db; cleanup: () => void } {
  const tempDir = mkdtempSync(path.join(tmpdir(), "oc-workflow-integrations-test-"));
